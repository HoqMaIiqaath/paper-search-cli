...
3. Prefer the `paper-research` MCP server. It mirrors the complete runtime `paper-search tools` catalog, so use the original CLI tool name when provider-specific parameters matter.
...
6. Only call the CLI directly if the MCP server is unavailable.
...
7. Keep Sci-Hub disabled unless the user explicitly enables it and confirms appropriate legal and institutional authorization.
