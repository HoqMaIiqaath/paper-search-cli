...
export function normalizePaper(
  ...
  return {
    id: paperId(title, year, doi, arxivId, pmid, semanticScholarId),
    title,
...
export function mergePapers(papers: PaperRecord[]): PaperRecord[] {
  ...
  for (const paper of papers) {
    ...
    merged.set(paper.id, {
      ...
      open_access:
        existing.open_access === "open" || paper.open_access === "open"
          ? "open"
          : existing.open_access,
      evidence_level:
        existing.evidence_level === "abstract" ||
        paper.evidence_level === "abstract"
          ? "abstract"
          : "metadata",
      sources: [...new Set([...existing.sources, ...paper.sources])],
    });
  }
